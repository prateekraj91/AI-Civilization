...
Run:  ./Jarvis/bin/python tests/test_simulation.py     (or: python3 -m tests.test_simulation)
"""
...
# The suite drives the project from the repo ROOT: put the root on the path (and keep a handle on
# it) so running the file directly, running it as `-m tests.test_simulation`, or hitting an
# editor's run button all resolve sim/llm/narrative/renderer — and the boundary tests that PARSE a
# source file find it wherever the shell happens to be.
_REPO_ROOT = _pathlib.Path(__file__).resolve().parents[1]
_sys.path.insert(0, str(_REPO_ROOT))

from llm import conversation
from llm import heuristic
from llm import llm
import main
from sim import world
from sim.agents import Agent
from sim.personality import Personality
from llm.strategy import Strategy, build_strategy_prompt, choose_action, get_personality
...
def test_strategy_validation_fallback() -> None:
    ...
    try:
        llm.PROVIDER = "random"
        from llm.strategy import VALID_STRATEGIES
        for _ in range(20):
...
def test_settlement_forms_with_enough_sustained_settlers_near_reliable_food() -> None:
    ...
    from sim import settlement
    from sim import world as _world
    _fresh_world()
...
def test_settled_agent_pulled_home_when_fed_not_when_starving() -> None:
    ...
    from sim import settlement
    from llm.strategy import SURVIVAL_HUNGER
    _fresh_world()
    create_world(size=16)
...
def test_guarded_knowledge_does_not_free_diffuse_but_sells() -> None:
    ...
    from sim import economy
    from sim import knowledge as kn
...
def test_leadership_effect_makes_a_led_settlement_more_cohesive_than_unled() -> None:
    ...
    not tax/law)."""
    from sim import leadership, settlement
...
def test_only_a_legitimate_leader_can_tax() -> None:
    ...
    from sim import leadership, taxation
...
def test_redistribution_lowers_within_settlement_gini_vs_untaxed() -> None:
    ...
    from sim import leadership, labor, taxation
...
def test_loyalty_repels_smaller_force_but_not_overwhelming_one() -> None:
    ...
    from sim import leadership, monarchy
...
def test_conquest_of_neighbour_makes_its_ruler_a_vassal() -> None:
    ...
    from sim import kingdoms, leadership, monarchy
    _realm_world()
...
def test_king_musters_loyal_vassal_but_not_broken_away_one() -> None:
    ...
    from sim import kingdoms, monarchy
    _realm_world()
    world_state["settlements"]["S001"] = {"id": "S001", "center": (5, 5), "members": {"King"}, "founded": 0}
...
def test_defeated_king_is_subjugated_into_multilevel_empire() -> None:
    ...
    poor = next(a for a in world_state["agents"] if a.name == "Poor")
    assert poor.relationships["Rich"]["trust"] == __import__("sim.kingdoms", fromlist=["*"]).LOYAL_TRUST, \
...
def test_imperial_tribute_cascades_through_subject_king_and_conserves_wealth() -> None:
    ...
    from sim import empire, kingdoms
...
def test_betrayal_drops_trust_8_latches_grudge_both_memories() -> None:
    ...
    from sim import alliance, trust
    _fresh_world()
...
# --- Renderer (Day 18) -----------------------------------------------------
_PACKAGES = {"sim", "llm", "narrative", "renderer"}


def _imported_modules(relpath: str) -> set[str]:
    """Every module a source file imports, as DOTTED names ('from sim import world' -> 'sim.world').

    The boundary tests below all need this. Since the packages landed, the old top-level read was
    useless — every module under sim/ came back as bare 'sim', so a renderer reaching for `empire`
    would have looked identical to one reading `world`. Full paths make a leak name itself.

    PARSES the file, never imports it, so these run with or without pygame installed. Rooted at the
    repo root, not the cwd, so they pass from wherever the suite was launched.
    """
    import ast
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            # `from sim import world` names a MODULE; `from sim.world import X` names one already.
            out.update(f"{node.module}.{a.name}" if node.module in _PACKAGES else node.module
                       for a in node.names)
    return out


def _project_imports(relpath: str) -> set[str]:
    """Just the PROJECT modules a source file imports ('sim.world'), third-party and stdlib dropped."""
    return {m for m in _imported_modules(relpath) if m.split(".")[0] in _PACKAGES}
...
def test_renderer_imports_only_state_reading_modules() -> None:
    ...
    """
    imported = _imported_modules("renderer/text_renderer.py")
    forbidden = {"llm.strategy", "sim.trust", "llm.conversation", "sim.alliance",
                 "sim.personality", "sim.agents", "llm.llm", "sim.god_mode", "sim.population"}
    ...
    assert _project_imports("renderer/text_renderer.py") <= {"sim.world"}, imported
...
def test_pygame_renderer_imports_only_state_reading_modules() -> None:
    ...
    """
    imported = _imported_modules("renderer/pygame_renderer.py")
    # Nothing out of sim/ or llm/ at all — the whole of both packages is decision logic to a renderer.
    forbidden = {m for m in imported if m.split(".")[0] in ("sim", "llm", "narrative")}
    assert not forbidden, f"pygame renderer imports decision logic: {forbidden}"
    ...
    allowed = {"__future__", "typing", "contextlib", "os", "sys", "time", "math", "textwrap",
               "collections", "pygame", "renderer.director"}
...
def test_showcase_scene_opens_in_a_standoff_that_must_break() -> None:
    ...
    from sim import empire, kingdoms, monarchy, scenario, world
    ...
    from sim import monarchy as _m
    for name in kings:
        king = next(a for a in state["agents"] if a.name == name)
...
def test_god_mode_imports_only_world_state_layers() -> None:
    ...
    imported = _imported_modules("sim/god_mode.py")
    forbidden = {"llm.strategy", "sim.trust", "llm.conversation", "sim.alliance",
                 "sim.personality", "llm.llm"}
    ...
    assert _project_imports("sim/god_mode.py") <= {"sim.world", "sim.population"}, imported
...
def test_dependent_consumes_parent_stockpile_and_does_not_produce() -> None:
    ...
    from sim import economy, knowledge, labor, lineage, monarchy, storage
    _lineage_world()
    ada = _settler("Ada", (5, 5))
    ben = _settler("Ben", (6, 5))
...
def test_old_age_death_uses_existing_death_path() -> None:
    ...
    from sim import lineage, population
    _lineage_world()
...
def test_m43_succession_does_not_inherit_loyalty() -> None:
    ...
    from sim import lineage, population, trust
...
def test_m43_succession_is_a_crisis_test() -> None:
    ...
    from sim import lineage, population, kingdoms, trust
...
def test_m43_extinct_line_dissolves_and_is_contestable() -> None:
    ...
    from sim import lineage, population, monarchy, kingdoms
...
def test_m43_dependent_heir_holds_seat_as_regent() -> None:
    ...
    from sim import lineage, population, monarchy
...
def test_legitimacy_buffers_extraction_grievance() -> None:
    ...
    from sim import discontent, trust
...
def test_victory_deposes_clears_title_and_breaks_kingdom_away() -> None:
    ...
    from sim import uprising, kingdoms
...
def test_expropriation_conserved_and_preempts_inheritance() -> None:
    ...
    from sim import uprising, population
...
def test_revolutionary_is_derived_from_risers_not_assigned() -> None:
    ...
    from sim import uprising, leadership, trust
...
def test_revolutionary_holds_by_consent_and_can_be_displaced() -> None:
    ...
    from sim import uprising, leadership
...
def test_revolution_devours_its_children() -> None:
    ...
    from sim import uprising, discontent, monarchy
...
def test_faith_forms_on_coherence_not_when_fractured() -> None:
    ...
    from sim import religion, beliefs
...
def test_prophet_emergence_logs_only_on_genuine_change() -> None:
    ...
    from sim import religion, trust, beliefs
    core = {beliefs.LAND_PROVIDES, beliefs.STRONGER_TOGETHER}
...
def test_aligned_ruler_generates_less_discontent_and_more_loyalty() -> None:
    ...
    from sim import religion, discontent, beliefs
    core = {beliefs.LAND_PROVIDES, beliefs.STRONGER_TOGETHER}
...
def test_defiant_king_erodes_vassal_loyalty_and_prophet_amplifies() -> None:
    ...
    from sim import religion, trust, beliefs, kingdoms
    core = {beliefs.LAND_PROVIDES, beliefs.STRONGER_TOGETHER}
...
def test_same_vs_foreign_rule_breeds_chronic_friction() -> None:
    ...
    from sim import culture, beliefs, discontent
...
def test_foreign_province_breaks_away_where_same_culture_holds() -> None:
    ...
    from sim import culture, beliefs, kingdoms, trust
    native = {beliefs.LAND_PROVIDES, beliefs.STRONGER_TOGETHER}
    foreign = {beliefs.STRONG_TAKE, beliefs.WEALTH_IS_VIRTUE}
...
def test_children_assimilate_but_adults_do_not() -> None:
    ...
    from sim import culture, beliefs
    native = {beliefs.LAND_PROVIDES, beliefs.STRONGER_TOGETHER}
    foreign = {beliefs.STRONG_TAKE, beliefs.WEALTH_IS_VIRTUE}
...
def test_heir_inherits_written_law_only_when_literate() -> None:
    ...
    from sim import writing, population
...
def test_metalworking_boosts_farm_yield() -> None:
    ...
    from sim import metallurgy, knowledge
...
def test_arms_multiply_force_in_battle() -> None:
    ...
    from sim import monarchy, metallurgy
...
def test_era_yield_curve() -> None:
    ...
    from sim import eras, knowledge
...
def test_pact_prevents_a_war_the_loop_would_launch_and_breaks_on_souring() -> None:
    ...
    from sim import diplomacy, empire
...
def test_intertrade_enriches_both_and_is_blocked_when_hostile() -> None:
    ...
    from sim import intertrade, diplomacy
...
def _make_subject(emperor, sk):
    from sim import kingdoms, trust
...
def test_fear_drives_coalition_overriding_stance() -> None:
    ...
    from sim import coalitions, diplomacy
...
def test_pooled_coalition_host_breaks_the_hegemon() -> None:
    ...
    from sim import coalitions, empire
...
def test_history_book_is_deterministic_named_and_pure() -> None:
    ...
    """
    import re as _re
    from narrative import chronicle, chronicle_book
    saved = llm.PROVIDER
    try:
...
def test_narrator_is_walled_off_from_the_structured_chronicle() -> None:
    ...
    import copy
    from narrative import chronicle, narrator
...
def test_director_imports_only_stdlib() -> None:
    ...
    """
    imported = _imported_modules("renderer/director.py")
...
def test_discontent_pulses_and_a_rising_takes_sides() -> None:
    ...
    """
    import copy, os as _os
    from sim import discontent as _discontent
    _os.environ.setdefault("SDL_VIDEODRIVER", "dummy")
    _os.environ.setdefault("SDL_AUDIODRIVER", "dummy")
    try:
        import pygame
...
def test_a_town_seethes_and_a_hoarding_seat_towers() -> None:
    ...
    try:
        ...
        from sim import uprising as _uprising
    except ImportError:
...
def test_no_two_wars_chain_in_one_turn() -> None:
    ...
    """
    from sim import empire, scenario, world
    world.create_world(size=26)
    state = world.world_state
    state["taxation_on"] = True
    scenario.apply(state, "showcase")
